...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.GUI:
            #### With debug GUI ########################################
            self.CLIENT = p.connect(p.GUI_SERVER) # p.connect(p.GUI, options="--opengl2")
    ...
    def step(self,
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            clipped_action = np.reshape(self._preprocessAction(action,target_angular_rate), (self.NUM_DRONES, 4))
        ...
        for _ in range(self.AGGR_PHY_STEPS):
            ...
            for i in range (self.NUM_DRONES):
                if self.PHYSICS == Physics.PYB.value:
                    self._physics(clipped_action[i, :], i)
                elif self.PHYSICS == Physics.DYN.value:
                    self._dynamics(clipped_action[i, :], i)
                elif self.PHYSICS == Physics.PYB_GND.value:
                    self._physics(clipped_action[i, :], i)
                    self._groundEffect(clipped_action[i, :], i)
                elif self.PHYSICS == Physics.PYB_DRAG.value:
                    self._physics(clipped_action[i, :], i)
                    self._drag(self.last_clipped_action[i, :], i)
                elif self.PHYSICS == Physics.PYB_DW.value:
                    self._physics(clipped_action[i, :], i)
                    self._downwash(i)
                elif self.PHYSICS == Physics.PYB_GND_DRAG_DW.value:
                    self._physics(clipped_action[i, :], i)
                    self._groundEffect(clipped_action[i, :], i)
                    self._drag(self.last_clipped_action[i, :], i)
                    self._downwash(i)
    ...
    def _downwash(self,
        ...
        for i in range(self.NUM_DRONES):
            ...
            if delta_z > 0 and delta_xy < 1.5: # Ignore drones more than 10 meters away
                ...
                if abs(downwash[2])> (0.028*10*2.5):
                    downwash[2] = -0.028*10*2.5
                p.applyExternalForce(self.DRONE_IDS[nth_drone],
                                     4,
                                     forceObj=downwash,
                                     posObj=[0, 0, 0],
    ...
    def _addObstacles(self):
        ...
        p.loadURDF(str(root_path)+"/catkin_ws/GammaSwarm/src/GammaSwarm/src/GammaSwarm/simulationSystems/obstacle.urdf",[-2/2,2/2,1],physicsClientId = self.CLIENT)
        p.loadURDF(str(root_path)+"/catkin_ws/GammaSwarm/src/GammaSwarm/src/GammaSwarm/simulationSystems/obstacle.urdf",[-1.1/2,2/2,1],physicsClientId = self.CLIENT)
        p.loadURDF(str(root_path)+"/catkin_ws/GammaSwarm/src/GammaSwarm/src/GammaSwarm/simulationSystems/obstacle.urdf",[-1.50/2,1.30/2,1],physicsClientId = self.CLIENT)
